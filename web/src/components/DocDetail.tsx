...
export default function DocDetail({ id, onClose, onUpdated }: { id: number | null; onClose: ()=>void; onUpdated?: ()=>void }) {
      ...
      <div className="flex-1 overflow-auto bg-[#0a0a0a]">
        {tab==="meta" && (
          <div className="p-5 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <a href={`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/library/documents/${id}/file`} target="_blank" className="rounded-xl border border-[#2f2f2f] bg-[#212121] px-4 py-2.5 text-sm text-white hover:bg-[#2f2f2f]">Open PDF ↗</a>
            </div>
            ...
            <div className="space-y-3 pt-3 border-t border-[#2f2f2f]">
              ...
              <div className="rounded-xl bg-[#171717] border border-[#2f2f2f] p-3 text-xs leading-relaxed">
                ...
                <div className="text-[#8e8e8e] mt-1">Hash <span className="font-mono text-[#ececec]">{doc.file_hash?.slice(0,16)}…</span> • {doc.ingestion_status} {doc.ingestion_error ? `• ${doc.ingestion_error.slice(0,120)}` : ""}</div>
                ...
                <div className="text-[#8e8e8e] break-all">Stored at <span className="font-mono text-[#ececec]">{doc.stored_path}</span></div>
              </div>
        ...
        {tab==="ask" && (
          <div className="p-5 space-y-4">
            ...
            {!askAns && <div className="text-sm text-[#8e8e8e]">Ask a question scoped to this document. Uses hybrid BM25+vector + cross-encoder rerank.</div>}
          </div>
        )}
        ...
        {tab==="cite" && (
          <div className="p-5 space-y-3">
            ...
            <div className="text-xs text-[#8e8e8e]">CSL rendering (backend citeproc-py). Set the default in Settings → Citation.</div>
              ...
              <div className="space-y-2">
                {references.references.map((r:any)=>(
                  <div key={r.number} className="rounded-xl border border-[#2f2f2f] bg-[#171717] p-3">
                    ...
                    {r.match && <div className="mt-1.5 text-xs text-emerald-300">→ in library: {r.match.title} ({Math.round(r.match.score*100)}%) <button onClick={()=>{ onClose(); }} className="underline">open drawer</button></div>}
                  </div>
                ))}
        ...
        {tab==="notes" && (
          <div className="p-5 space-y-3">
            <div className="text-sm text-[#8e8e8e]">Notes & highlights for this document. Highlights are stored as structured data in SQLite, not burned into the PDF.</div>
...
function AnnotationsPanel({ docId }: { docId:number }) {
