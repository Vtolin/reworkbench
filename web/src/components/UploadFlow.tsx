...
function buildEdit(candidate: any, preview: any) {
  const e = preview?.extracted || {};
  return {
    title: candidate?.title || e.title || "",
    authors: (candidate?.authors || []).join(", "),
    year: candidate?.year ?? e.year ?? "",
    doi: candidate?.doi || e.doi || "",
    jurisdiction: e.jurisdiction || "",
    document_type: candidate?.document_type || e.document_type || "",
    ...
    collection_ids: preview?.suggested_collection ? [preview.suggested_collection.id] : [],
  };
}
...
export default function UploadFlow({ onDone }: { onDone?: ()=>void }) {
  ...
  const startPreview = async (f: File) => {
    ...
    setLoading(true);
    try {
      const r = await api.ingestPreview(f);
      setPreview(r);
      setCandidates(r.metadata_candidates || []);
      const fetched = r.metadata_proposal && r.metadata_proposal.source !== "local"
        ? r.metadata_proposal : null;
  ...
  const confirm = async() => {
    ...
    try{
      const body = {
        ...
        citation_metadata: activeCandidate?.extras || {},
        metadata_source: activeCandidate?.source || "local",
        metadata_confidence: activeCandidate ? (activeCandidate.confidence ?? null) : null,
        metadata_verified: true,
        collection_ids: edit.collection_ids || [],
        tag_ids: [],
        action: "accept",
      };
      const r = await api.ingestConfirm(body);
      setResult(r);
      onDone?.();
    } catch(e:any){ setError(e.message); }
    finally{ setLoading(false); }
  ...
  const proposal = preview?.metadata_proposal;
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border-2 border-dashed border-[#2f2f2f] bg-[#0a0a0a] p-6 lg:p-8 text-center">
        ...
        <div className="mt-3 font-medium text-white">Drop a PDF / DOCX / MD here</div>
        <div className="text-sm text-[#8e8e8e]">Local parsing • SHA-256 dedup • OpenAlex metadata lookup • No file moves until you click Accept</div>
        <label className="mt-4 inline-flex cursor-pointer rounded-xl bg-white text-black px-5 py-2.5 text-sm font-medium hover:bg-[#ececec]">
          Choose file
          <input type="file" className="hidden" accept=".pdf,.docx,.xlsx,.pptx,.txt,.md,.csv,.html,.htm,.rtf" onChange={e=> e.target.files?.[0] && startPreview(e.target.files[0])} />
        </label>
        {file && <div className="mt-3 text-xs font-mono text-[#8e8e8e] break-all">{file.name} • {(file.size/1024/1024).toFixed(2)} MB</div>}
        {loading && <div className="mt-3 text-sm text-white">Analyzing… extracting metadata, checking OpenAlex, duplicates, classifying</div>}
        ...
        <div className="rounded-2xl border border-[#2f2f2f] bg-[#0a0a0a] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#2f2f2f] bg-[#171717] flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              ...
              <div className="text-sm text-[#b4b4b4] mt-1">Nothing moves until you click <b className="text-white">Accept</b>. Edit anything below.</div>
            </div>
            <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium border self-start ${preview.decision==="auto_suggest" ? "bg-emerald-950 border-emerald-800 text-emerald-300" : preview.decision==="confirm" ? "bg-amber-950 border-amber-800 text-amber-300" : "bg-[#212121] border-[#2f2f2f] text-[#8e8e8e]"}`}>{preview.decision} • {Math.round(preview.collection_confidence*100)}%</span>
          </div>

          {/* Metadata verification card (Phase 1): source + confidence, Accept/Edit/Reject */}
          {proposal && (
            <div className={`mx-5 mt-4 rounded-xl border p-3 ${fetchedActive ? "bg-emerald-950/30 border-emerald-800" : "bg-[#171717] border-[#2f2f2f]"}`}>
              {fetchedActive ? (
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  {preview.metadata_error && <span className="text-xs text-[#5f5f5f]">{preview.metadata_error}</span>}
                </div>
              )}
            </div>
          )}
          ...
          {preview.is_duplicate && (
            <div className="mx-5 mt-4 rounded-xl bg-red-950/30 border border-red-900 p-3 text-sm text-red-300">
              ...
              <ul className="list-disc ml-5 mt-1 break-words">
                {preview.duplicates.map((d:any)=><li key={d.id}>#{d.id} {d.title} — {d.label} ({Math.round(d.confidence*100)}%)</li>)}
              </ul>
            </div>
          )}
          ...
          <div className="p-5 grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {collections.map(c=>{
                  ...
                  return <button key={c.id} onClick={()=>setEdit({...edit, collection_ids: on? edit.collection_ids.filter((x:number)=>x!==c.id) : [...(edit.collection_ids||[]), c.id]})} className={`rounded-full px-3 py-1.5 text-xs border flex items-center gap-1.5 ${on? "bg-white text-black border-white":"bg-[#171717] border-[#2f2f2f] text-[#ececec]"}`}><span className="h-2 w-2 rounded-full" style={{background:c.color}} />{c.name}</button>
                })}
                {collections.length===0 && <span className="text-sm text-[#5f5f5f]">No collections — create one in Settings or Library.</span>}
              </div>
              {preview.suggested_collection && <div className="mt-2 text-xs text-[#8e8e8e]">Suggested: <b className="text-white">{preview.suggested_collection.name}</b> — {preview.collection_reason} • {Math.round(preview.collection_confidence*100)}% confidence</div>}
            </div>
            <div className="md:col-span-2 rounded-xl bg-[#171717] border border-[#2f2f2f] p-3">
              ...
              <div className="text-xs font-mono mt-2 whitespace-pre-wrap leading-relaxed text-[#b4b4b4] break-words">{preview.text_snippet?.slice(0,900)}</div>
              <div className="text-xs text-[#5f5f5f] mt-2 break-all">Hash {preview.file_hash.slice(0,16)}… • {preview.page_count ?? "?"} pages • {preview.mime_type}</div>
            </div>
          </div>
          ...
          <div className="px-5 py-4 border-t border-[#2f2f2f] flex flex-col sm:flex-row gap-3 bg-black">
            ...
            <button onClick={async()=>{ await api.ingestConfirm({ temp_id: preview.temp_id, title:"", action:"cancel" }); setPreview(null); setFile(null); }} className="rounded-xl border border-[#2f2f2f] bg-[#212121] px-6 py-3 text-sm text-white hover:bg-[#2f2f2f]">Cancel</button>
          </div>
        </div>
      )}
      ...
      {result && (
        <div className="rounded-2xl border border-emerald-900 bg-emerald-950/30 p-4">
          <div className="text-sm font-medium text-emerald-300">✓ Ingested as #{result.document.id} — {result.document.title}</div>
          {result.document.metadata_source && <div className="text-xs mt-1 text-emerald-400/70">Metadata source: {result.document.metadata_source}{result.document.metadata_confidence != null ? ` • ${Math.round(result.document.metadata_confidence*100)}%` : ""}</div>}
          {result.indexing_error && <div className="text-xs mt-2 text-amber-300 bg-amber-950/30 border border-amber-900 rounded-lg p-2 break-all">Indexing warning: {result.indexing_error.slice(0,400)}</div>}
