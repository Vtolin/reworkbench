...
export default function ProjectDetail(){
  ...
  const [citFor, setCitFor] = useState<number | null>(null);
  const [citDoc, setCitDoc] = useState<number | null>(null);
  ...
  const load = async()=>{
    ...
    setProj(p);
    api.projectClaims(Number(id)).then(setClaims).catch(()=>{});
  };
  ...
  const ask = async()=>{
    ...
    setLoading(true);
    try{
      const r = await api.ask({ query, document_ids: (proj.documents||[]).map((d:any)=>d.id), project_id: Number(id), session_id: `project-${id}` });
      setAns(r);
  ...
  const addEvidence = async()=>{
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/research/projects/${id}/evidence`, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ claim: evidenceClaim, quoted_evidence: evidenceQuote }) });
    setEvidenceClaim(""); setEvidenceQuote(""); load();
  };

  const addClaim = async()=>{
    if(!newClaim.trim()) return;
    await api.createClaim({ project_id: Number(id), text: newClaim });
    setNewClaim("");
    api.projectClaims(Number(id)).then(setClaims).catch(()=>{});
  };

  const linkEvidence = async(cid: number)=>{
    if(!citDoc) return;
  ...
  const deleteProject = async()=>{
    ...
    try{ await api.deleteProject(Number(id)); window.location.href = "/projects"; } catch(e:any){ alert(e.message); }
  };
                ...
                {claims.map(claim=>(
                  <div key={claim.id} className="rounded-xl border border-[#2f2f2f] bg-black p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <button onClick={async()=>{ await api.deleteClaim(claim.id); api.projectClaims(Number(id)).then(setClaims).catch(()=>{}); }} className="text-[#5f5f5f] hover:text-red-400 text-xs">✕</button>
                    </div>
                    ...
                    {citFor===claim.id ? (
                      <div className="mt-2 flex flex-wrap gap-2 items-center">
                        <select value={citDoc ?? ""} onChange={e=>setCitDoc(Number(e.target.value))} className="rounded-xl border border-[#2f2f2f] bg-[#171717] px-2 py-1.5 text-xs text-white">
            ...
            <div className="rounded-2xl border border-[#2f2f2f] bg-[#0a0a0a] p-4 lg:p-5">
              ...
              <div className="space-y-2 mt-3">
                {(proj.queries||[]).map((qu:any)=>{
                  ...
                  try{
                    const parsed = qu.sources_json ? JSON.parse(qu.sources_json) : null;
