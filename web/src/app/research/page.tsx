...
export default function ResearchPage(){
  ...
  const [summarizeDoc, setSummarizeDoc] = useState<number | null>(null);
  const [summarizeLoading, setSummarizeLoading] = useState(false);
  const [armedExport, setArmedExport] = useState<"pdf" | "html" | null>("pdf");
  const [lastSummaryDoc, setLastSummaryDoc] = useState<number | null>(null);
  ...
  const [matrixIds, setMatrixIds] = useState<number[]>([]);
  ...
  const { activeConversation, addMessage, removeMessage, updateMessage, truncateAfter, clearActive } = useChat();
  ...
  const [streaming, setStreaming] = useState<{ answer: string; thinking: string | null; raw: string; sources: any[] | null; retrieved: any[] | null } | null>(null);
  ...
  // backend conversation memory is per-session (keyed by chat id):
  // switching chats just switches sessions, no clearing needed.
  const sessionId = activeConversation?.id ?? "default";
  ...
  const sendAsk = async (
    queryText: string,
    opts: { scopeIds?: number[]; broad?: boolean; thinking?: boolean; hybrid?: string; memory?: boolean },
    ...
    const handleFallback = async () => {
      ...
      didFallback = true;
      try{
        const r = await api.ask({ query: queryText, document_ids: scope.length? scope : undefined, broad: broadVal, thinking: thinkingVal, use_memory: memoryVal, hybrid_mode: hybridVal, session_id: sessionId });
        addMessage({
    ...
    const finalizeStopped = () => {
      ...
      if (acc.sawThinkingEvent) {
        ...
        // pre-explicit-events backend fallback: parse <think> out of the raw text
    ...
    try{
      await api.askStream(
        { query: queryText, document_ids: scope.length? scope : undefined, broad: broadVal, thinking: thinkingVal, use_memory: memoryVal, hybrid_mode: hybridVal, session_id: sessionId },
        {
          onMeta: (data) => {
  ...
  const doSummarize = async()=>{
    ...
    const finishSummary = (r: any)=>{
      ...
      // pipeline export: the file was rendered server-side from this same
      // summary (no second LLM run) - download it and confirm.
      if (r.export?.export_content) {
        const kb = Math.max(1, Math.round((r.export.size_bytes || 0) / 1024));
        downloadBlob(b64ToBlob(r.export.export_content, r.export.format === "html" ? "text/html" : "application/pdf"), r.export.filename);
        flashExportNote(true, `Exported ${r.export.filename} (${kb} KB) — verified ✓`);
      } else if (r.export_error) {
        flashExportNote(false, `Summary done, but ${armed?.toUpperCase()} export failed: ${r.export_error}`);
      }
    };
    ...
    try{
      // live pipeline log via SSE; falls back to the plain call if the
      // stream itself can't start (the summary result is identical).
      let streamed = false;
      try{
        await api.summarizeStream(
          { document_id: summarizeDoc, ...(armed ? { export_format: armed } : {}) },
          {
            onStatus: (stage, detail, progress) => setSummarizeStatus({ stage, detail, ...progress, since: Date.now() }),
        ...
        if (!streamed) {
          const r = await api.summarize({ document_id: summarizeDoc, ...(armed ? { export_format: armed } : {}) });
  ...
  // explicit thinking/answer streams from the backend; fall back to parsing
  // <think> tags out of the raw text if talking to an older backend that
  // still sends everything through token events.
      ...
      <div className="flex-1 min-w-0 flex flex-col h-full relative z-0 min-h-0">
          ...
          <div className="max-w-4xl mx-auto px-4 lg:px-6 py-3">
            <div className="flex items-center gap-2">
              ...
              <div className="ml-auto flex items-center gap-2">
                {mode==="ask" && (
                  <>
                    <button onClick={()=>setThinkingMode(!thinkingMode)} className={`hidden sm:flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium border transition ${thinkingOn ? pillOn : pillOff}`} title="Thinking mode: model reasons step-by-step (Ollama think=true) before answering. Toggle on for hypotheses.">
                    ...
                    <button onClick={()=>{ clearActive(); api.clearMemory(sessionId).catch(()=>{}); }} className="text-xs text-[#8e8e8e] hover:text-white px-2 hidden sm:block">Clear chat</button>
                  </>
                )}
              </div>
            </div>
            ...
            {mode==="summarize" && (
              <div className="mt-3 flex gap-2 items-center flex-wrap">
                <select value={summarizeDoc ?? ""} onChange={e=>setSummarizeDoc(e.target.value? Number(e.target.value): null)} className="flex-1 min-w-[200px] rounded-xl bg-[#212121] border border-[#2f2f2f] px-3 py-2 text-sm text-white">
            ...
            {messages.length===0 && !streaming ? (
              <div className="min-h-[60vh] flex flex-col items-center justify-center text-center py-16">
                ...
                <p className="text-sm text-[#8e8e8e] mt-2 max-w-md">
                  {mode==="ask" ? "Ask any question against your library. Toggle Thinking for hypotheses or unanswerable questions — uses the Ollama model's built-in reasoning (think=true)." : mode==="compare" ? "Select ≥2 documents and ask how they relate. Each is retrieved separately and labeled." : mode==="summarize" ? "Pick a document. We run the full map→reduce→synthesis pipeline and return verbatim facts." : mode==="matrix" ? "Select documents and generate a matrix: paper, method, dataset, findings, limitations. Export to CSV, XLSX or Markdown." : "Select ≥2 documents and ask what the literature agrees on, disagrees on, or where the gaps are — answers are attributed per source."}
                </p>
                {mode==="ask" && (
                      ...
                      {m.role==="assistant" && m.thinking && (
                        <details className="mb-3 rounded-xl bg-[#0a0a0a] border border-[#2f2f2f] overflow-hidden group/thinking">
                          <summary className="px-3 py-2 text-xs font-medium text-[#ab68ff] cursor-pointer select-none flex items-center gap-2 list-none">
                            ...
                            <span className="text-[#5f5f5f] font-normal hidden sm:inline">— model reasoning (Ollama)</span>
                ...
                {streaming && (
                  <div className="flex gap-3 justify-start">
                    ...
                    <div className="max-w-[85%] sm:max-w-[78%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap break-words bg-transparent text-[#ececec] border border-[#2f2f2f]/50">
                      {/* live pipeline status: what the backend is doing before tokens arrive */}
        ...
        <footer className="shrink-0 border-t border-[#2f2f2f] bg-black">
          <div className="max-w-3xl mx-auto px-4 py-4">
            ...
            {mode==="summarize" && (
              <div className="text-xs text-[#8e8e8e] text-center py-2">Select a document above and click Summarize. Full pipeline: stuff/map→reduce→synthesis + verbatim facts.</div>
            )}
            {mode==="matrix" && (
              <div className="text-xs text-[#8e8e8e] text-center py-2">Select documents above and click Build matrix. Each row is extracted by the local model; export as CSV / XLSX / Markdown.</div>
            )}
            {mode==="synthesis" && (
              <div className="rounded-2xl bg-[#212121] border border-[#2f2f2f] flex items-end gap-2 p-2">
            ...
            <div className="text-[11px] text-[#5f5f5f] text-center mt-2 px-2">
              {chatCfg?.chat ? `${chatCfg.chat.model} • ${chatCfg.chat.num_ctx} ctx` : "Ollama"} • Hybrid BM25+vector • Cross-encoder rerank • {thinkingOn ? "Thinking on" : "Thinking off"} • {memoryOn ? "Memory on" : "Memory off"} • {hybridMode!=="off" ? `Hybrid source ${hybridMode}` : "Strict grounding"}
            </div>
          </div>
        </footer>
      </div>
