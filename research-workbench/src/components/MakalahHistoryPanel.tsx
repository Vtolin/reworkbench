    ...
    <div className="flex flex-col h-full bg-[#0a0a0a]">
      <div className="p-3 border-b border-[#2f2f2f]">
        <button
          ...
          className="w-full rounded-xl bg-white text-black py-2.5 text-sm font-medium hover:bg-[#ececec]"
        >
          + New makalah
        </button>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1 no-scrollbar">
        {sorted.length === 0 && (
          <div className="text-xs text-[#5f5f5f] p-3 text-center">No makalah yet</div>
        )}
        {sorted.map((d) => {
          ...
          return (
            <div
              ...
              className={`group rounded-xl px-3 py-2.5 cursor-pointer border ${d.id === activeId ? "bg-[#212121] border-[#2f2f2f]" : "border-transparent hover:bg-[#171717]"}`}
            >
              <div className="flex items-center gap-2">
                <span className="text-xs text-[#8e8e8e] shrink-0">✎</span>
                <span className="text-sm truncate flex-1 text-[#ececec]">{draftTitle(d)}</span>
                <button
