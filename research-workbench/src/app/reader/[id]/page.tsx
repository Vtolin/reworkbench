...
export default function ReaderPage(){
  ...
  useEffect(()=>{ if(!id) return; api.getDocument(String(id)).then(async d=>{ setDoc(d); try{ setPdfUrl(await api.documentFileUrl(d)); }catch{} }).catch(()=>{}); },[id]);
  useEffect(()=>{ if(id) api.annotations(String(id)).then(setHighlights).catch(()=>{}); },[id]);

  const addHighlight = async(color="#facc15")=>{
    if(!selection.trim() || !id) return;
    await api.createAnnotation(String(id), { page: 1, selected_text: selection, note, color });
    setSelection(""); setNote(""); const h=await api.annotations(String(id)); setHighlights(h);
  };
  const askSelection = async()=>{
    if(!selection.trim() || !id) return;
    const r = await api.ask({ query: `Explain: "${selection}"`, document_ids: [String(id)] });
      ...
      <div className="flex flex-1 min-h-0 flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-[360px] shrink-0 border-t lg:border-t-0 lg:border-l border-[#2f2f2f] bg-[#0a0a0a] flex flex-col max-h-[50vh] lg:max-h-none">
          <div className="p-4 border-b border-[#2f2f2f]">
            ...
            <div className="text-xs text-[#8e8e8e] break-words">{highlights.length} highlights — stored as structured data in the shared workspace</div>
          </div>
          <div className="flex-1 overflow-auto p-3 space-y-2">
            {highlights.map(h=>(
              <div key={h.id} className="rounded-xl border p-3" style={{background: (h.color as string)+"20", borderColor: "#2f2f2f"}}>
                <div className="text-xs text-[#8e8e8e]">p. {h.page as number} • {new Date(h.created_at as string).toLocaleString()}</div>
                <div className="text-sm mt-1 text-white break-words">“{h.selected_text as string}”</div>
                {Boolean(h.note) && <div className="text-sm mt-1 text-[#ececec] bg-black rounded-lg px-2 py-1 border border-[#2f2f2f] break-words">{h.note as string}</div>}
                <div className="mt-2 flex gap-1 flex-wrap">
                  <button onClick={async()=>{ const r=await api.ask({ query:`Explain: ${h.selected_text}`, document_ids:[String(id)] }); alert(r.answer.slice(0,1200)); }} className="text-xs rounded-full border border-[#2f2f2f] bg-[#212121] px-2 py-1 text-white">Ask AI</button>
                </div>
              </div>
            ))}
