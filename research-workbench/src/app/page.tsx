  ...
  return (
    <div className="flex flex-1 min-h-0">
      <div className="flex-1 min-w-0 flex flex-col bg-black">
        <Topbar title="Library" subtitle={filteredInfo || `${total} documents • shared workspace library`} actions={
        ...
        <div className="px-4 lg:px-6 py-4 space-y-4 flex-1 overflow-auto">
                    ...
                    <tr key={d.id} className={`hover:bg-[#171717] ${selected===d.id? "bg-[#212121]":""}`}>
                      <td className="px-4 py-3 align-top">
                        <button onClick={()=>setSelected(d.id)} className="text-left w-full group">
                          <div className="font-medium leading-tight text-white group-hover:text-[#ab68ff] wrap-break-word line-clamp-2">
                            {d.title}
                            {d.status && d.status !== "approved" && (
                              <span className="ml-2 rounded-full bg-amber-950 border border-amber-800 px-2 py-0.5 text-[11px] text-amber-300 align-middle">{d.status}</span>
                            )}
                          </div>
                      ...
                      <td className="px-4 py-3 align-top text-right">
                        <div className="inline-flex gap-1">
                          ...
                          <button onClick={()=>remove(d)} className="rounded-lg border border-red-900/50 text-red-400 bg-[#171717] px-2.5 py-1 text-xs hover:bg-red-950/30">Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="lg:hidden space-y-3">
            {docs.map(d=>(
              <div key={d.id} onClick={()=>setSelected(d.id)} className={`rounded-2xl border p-4 cursor-pointer ${selected===d.id? "bg-[#212121] border-white/20" : "bg-[#0a0a0a] border-[#2f2f2f] active:bg-[#171717]"}`}>
                <div className="font-medium text-white leading-tight wrap-break-word">
                  {d.title}
                  {d.status && d.status !== "approved" && (
                    <span className="ml-2 rounded-full bg-amber-950 border border-amber-800 px-2 py-0.5 text-[11px] text-amber-300 align-middle">{d.status}</span>
                  )}
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button onClick={(e)=>{ e.stopPropagation(); remove(d); }} className="rounded-xl border border-[#2f2f2f] px-4 py-2 text-sm text-[#8e8e8e]">Delete</button>
                </div>
              </div>
            ))}
            {docs.length===0 && (
          ...
          <div className="text-xs text-[#5f5f5f] pb-4">Showing {docs.length} of {total} • <span className="font-mono">shared workspace library</span></div>
        </div>
      </div>
