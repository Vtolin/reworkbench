...
- `CloudProvider` (`src/lib/ai/cloud.ts`) — browser → Vercel proxy → provider with the member's own key. No thinking box (cloud path never sets it). Thinking budget/level ride along in the proxy request for backends that support them; other providers ignore them.
- Provider registry + proxy helpers (`src/lib/ai/providers.ts`): `openai`/`deepseek`/`google` share the OpenAI-compatible dialect (Google via `generativelanguage.googleapis.com/v1beta/openai`); `anthropic` is native. **Google thinking:** `thinking_budget` (Gemini 2.5, numeric incl. 0-disable) vs `thinking_level` (Gemini 3, categorical) overlap and are never sent together — `buildGoogleExtraBody` picks by model family. Proxy failures return the upstream message (truncated, key-free) instead of a bare status.
...
- **Quality report:** structure complete, citation integrity %, unsupported list (incl. salvage flags and generation errors), redundant section pairs (duplication detector), `ai_filled` model-bridged paragraph count, missing/unused references.
