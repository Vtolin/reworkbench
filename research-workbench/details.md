...
`AIProvider` (`src/lib/ai/types.ts`): `chat(messages, {model, temperature, numCtx, numPredict, thinking, thinkLevel, thinkingBudget, jsonMode, signal, onToken, onThinking})`, `embed(text)`. `numPredict` is the Ollama output cap and is forwarded as `max_tokens` on the cloud proxy; `thinking:false` is an explicit cold switch (never dropped); `jsonMode:true` requests `response_format:{type:"json_object"}` on OpenAI-compat providers. Two backends:

- `OllamaProvider` (`src/lib/ai/ollama.ts`) — browser → `localhost:11434` directly. Non-streaming and streaming-NDJSON paths. **Always sends an explicit `think: true/false`** (reasoning models such as qwen3 think natively when the field is omitted and can burn the whole `num_predict` budget on thinking, returning empty content — verified live on `qwen3.5:4b`: omitted → 1024 thinking tokens + 0 answer bytes; `think:false` → clean answer in ~1.6s). When thinking is on, an effort level (`minimal|low|medium/high/max`, `minimal` = closest-to-off on Gemini 3, brief-`low` on Ollama) may be sent instead of `true` as a server-side trace bound. Numeric thinking budgets are NOT a stable Ollama feature (HTTP 400 on 0.33.3) — token budgets are enforced via prompt instruction + `num_predict` accounting (see Makalah). On HTTP 400 the field is retried omitted once (old-server fallback; the `think:true` case additionally reports "not supported" into the thinking box instead of failing). Spontaneous `<think>` tags with the toggle off are stripped silently, never surfaced.
- `CloudProvider` (`src/lib/ai/cloud.ts`) — browser → Vercel proxy → provider with the member's own key. Forwards the explicit `thinking` switch (false = cold), the output cap as `maxTokens`, effort level, and `jsonMode`; surfaces a native `thinking` trace when the proxy returns one so two-phase drafting works on cloud too. Other providers ignore what they don't support.
- Provider registry + proxy helpers (`src/lib/ai/providers.ts`): `openai`/`deepseek`/`google` share the OpenAI-compatible dialect (Google via `generativelanguage.googleapis.com/v1beta/openai`); `anthropic` is native. **Google thinking:** `thinking_budget` (Gemini 2.5, numeric incl. 0-disable) vs `thinking_level` (Gemini 3, categorical incl. `minimal` closest-to-off) overlap and are never sent together — `buildGoogleExtraBody` picks by model family. Explicit `thinking:false` maps to `minimal` (3.x) / `budget:0` (2.5) instead of the provider default (`medium` on 3.5-flash). The proxy omits `temperature` for Gemini 3 (ignored by the provider), forwards `max_tokens` (clamped 128–16384; Anthropic 256–8192), and sends `response_format:{type:"json_object"}` when `jsonMode` is set. Proxy failures return the upstream message (truncated, key-free) instead of a bare status.

`InferenceContext` (device-local, `rw.inference_settings.v1`): provider `ollama|cloud`, Ollama tag (default `gemma4:26b-a4b-it-qat`), `numCtx` (default 32768), `temperature` (default 0.0), `embedMode` `local|server`, cloud provider/model (default `openai`/`gpt-4o-mini`), `summarizeMethod` (`stuff|map_reduce`), per-stage overrides (`mapStage`, `reduceStage`, `synthesisStage` — each `inherit|ollama|cloud`), Makalah overrides (`makalahOutlineStage`, `makalahSectionStage`, `makalahThinking` default off, `makalahThinkLevel` default `low`, `makalahThinkingBudget` default 1024, `makalahNumPredict` default 3072, clamped 256–16384). `lib/ollama/detect.ts` powers Settings → Auto-detect. Keys: AES-GCM at rest (`lib/ai/keys.ts`, `ai_credentials` owner-only RLS), never `NEXT_PUBLIC_*`.
...
- **Section loop (1–2 LLM calls per subsection):** focus-first query (distinctive `title + focus` span doubled against shared chapter/topic terms) → evidence allocator: widen fetch → exclude passages owned by other sections → MMR-select 4 (relevance + intra-set diversity, deterministic, temperature-0 safe) → starvation backfill in rank order, never empty → keep 4 passages (0-passage fallback: chapter + topic across all selected) → section call (target words, citation style, grounding mode, JSON-only) → code validator. Synthesis chapters (`synthesis_only` flag, else last position in multi-chapter outlines) skip the allocator: their evidence is the already-cited passages of drafted sections (one representative passage per drafted section first, then most-cited fill, capped 8), so Penutup can only recombine — never fresh evidence. Stored passages always equal the MMR-selected set (validator-aligned). Regenerate is idempotent by design (temp 0 + same evidence); divergence comes from changed inputs: per-section **↻ Bukti baru** (excludes own + others' passages, injects the verbatim negative list of the most-similar section, warns on pool exhaustion) and an opt-in 1× in-loop auto-retry on similarity ≥ bar (fresh evidence + negative list; no temperature bump — Gemini 3 ignores it). A post-generation correction retry (max 1 extra call) also fires on deterministic defects: unknown ids, page mismatches, empty/absurd citations, source-gap meta-sentences in prose. Grounding modes (Setup selector + Drafting toggle, persisted per draft): `off` strict, `15/85` coherence bridging (default), `30/70` synthesis framing; hybrid paragraphs carry empty citations and are counted as `ai_filled` in the quality report instead of being faked as sourced. The PENTING rule forbids "tidak ditemukan dalam sumber"-style meta-sentences in paragraph text (gaps go to `gaps`). Document context (full outline + COVERED bullet list capped ~600 chars + verbatim negative list + Penutup synthesis contract for the last chapter + scope guard) is injected per section against 1.1≈1.2≈2.1 duplication; deliberation cross-checks passages against COVERED whenever prior context exists (provider-agnostic, not gated on thinking). Step-3 cards show a P7 evidence-overlap indicator (`n/4 shared dengan X.Y`, amber past half) separating retrieval-caused from model-caused duplication. Thinking runs ONLY here — never outline, never claim-check. When on, drafting is two-phase: deliberate with thinking at the configured level and `num_predict` == thinking budget (a hard cap, so the trace can never eat the answer; a valid early JSON draft short-circuits), then answer cold with the full output cap and the trace injected as context. Sequential with progress; Stop aborts the in-flight model call (AbortController threaded to generation) and keeps the last-good text — regenerate never clears the previous draft first, so reload/Stop cannot destroy it. Draft autosave surfaces a banner when browser storage quota fails instead of silently losing work. No auto-jump to Result on failures; per-section regenerate + text edit (edit-save revalidates citations, clears stale verdicts) + on-demand claim check (page-aware evidence filter). Quota errors surface an actionable hint (switch section stage to Ollama, regenerate).
- **Anti-leakage attribution:** passages reach the model labeled with short opaque aliases (`S1`, `S2` — never raw UUIDs, which read as citation keys and get pasted into prose), with an explicit Bad/Good negative example in the prompt; citations map back to real ids on return (case-insensitive, bracket-tolerant, embedded page markers extracted; unknown aliases survive so the validator flags them, and the UI renders them as `Sumber S2 (tak terpetakan)` instead of a fake title). A regex strip layer (`stripLeakedCitations`: UUID shapes, multi-alias `[S2; S5]`, bare `S2, h. 47` echoes, `(S1, p./h./halaman N)` echoes in both bracket and paren form, `(Author et al., YYYY)` echoes — while preserving legit refs like `(UUD 1945)`) plus exact title-echo stripping (`stripTitleEchoes`, long titles only) runs before anything reaches the UI.
...
- **JSON hardening:** explicit `think:false` on outline/claim-check/cold-section calls (Gemini maps to `minimal`/0, see §1.5) plus `response_format:{type:"json_object"}` on cloud, fence/prose stripping, greedy-brace extraction, trailing-comma repair, one bounded correction retry showing the model its own bad output; empty/think-only output fails fast with an actionable message. A post-generation correction pass (max 1 extra call per section) additionally repairs deterministic defects — unknown source ids, page mismatches, empty/absurd citations, source-gap meta-sentences in prose — keeping whichever draft has fewer defects. Final section fallback preserves raw text with **zero** citations plus an `[unstructured-fallback]` marker that forces a quality-report entry — provider/network errors are never salvaged, they stay hard errors.
- **Bibliography:** deterministic, from stored metadata (APA-ish plain text via `docToCslItem` + `renderCitationPlain`; no-author items lead with the title, trailing periods de-duplicated; missing titles fall back to filename, then id stub — a cited source always yields an entry). Entries that still look broken (leading `(year)`, `..`) are flagged as `malformed_references` in the quality report instead of silently shipping. References auto-top-up (`ensureReferences`) on preview/export/copy, so per-section drafting can never leave Daftar Pustaka silently empty. Every entry is hand-editable in Step 4 (badge `manual`, persisted per draft); refresh/top-up only adds missing ids and never overwrites manual text.
...
- **Quality report:** structure complete, citation integrity % (— when zero citations), unsupported list (incl. salvage flags and generation errors), redundant section pairs (content-word similarity 0.88, 0.94 for Penutup-involved pairs), `ai_filled` model-bridged paragraph count, page-mismatch list (skipped for sources without page info) + soft missing-pages list (citations dropping pages on verifiable sources), missing/unused references. Outline gate (`findOutlineOverlaps`, Step 2): content-word scoring, 0.78 same-role / 0.82 cross-role, top-3 display.
...
- **Export:** PDF via print view (cover, TOC, sections with inline `[title, h. X]` cites, gap notes, Daftar Pustaka/References by language — QA report stays in-UI by design), plus Copy-as-Markdown. Export/copy is gated: missing or salvaged (citation-less fallback) sections block it with an itemized message instead of shipping a holey paper.
...
Chat card (provider/model/ctx/temp/embed mode/autodetect/cloud BYOK with live model list), Summarization pipeline card (method + map/reduce/synthesis overrides), **Makalah pipeline card** (outline/section overrides, thinking toggle + level/budget, output cap), Citation card, Import/Export card, Workspace card, Model-configuration card, Privacy, Storage (Supabase info + workspace JSON export), shortcuts reference. Everything inference-shaped is device-local.
...
- **Thinking**: explicit `think` flag for Ollama + `<think>` parsing fallback; cloud thinking traces are plumbed through the proxy and used internally by Makalah two-phase drafting (the Ask thinking box stays Ollama-only). Native-reasoning models are force-disabled unless the toggle/stage enables them (live-verified on qwen3.5:4b).
...
- **Settings**: Chat card → this-device inference (provider/model/ctx/temp/embed mode/cloud key/autodetect); Makalah card → stage overrides + thinking toggle/level/budget + output cap; Import/Export fully client-side (imports become pending records); Watch-folder card replaced by Workspace card (no daemon, by design); Storage card → Supabase info + workspace JSON export (replaces SQLite backup file); Privacy card rewritten for the new model.
